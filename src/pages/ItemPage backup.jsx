...
export default function ItemsPage() {
  ...
  useEffect(()=>{
    ...
    async function queryByGroup() {
       try {
        const response = await fetch(
          `${API_URL}/api/db2/items?group=${groupQuery}`,
          { signal: controller.signal }
        );
  ...
  return (
    <div className=" bg-black/20 h-1/2 w-full"> 
      {/* <Header></Header> */}
  ...
  {/* modal div */}
      ...
      {/* items card */}
      <div
        className={`w-full px-20 py-4 flex gap-4 flex-wrap justify-evenly pt-36 z-0 min-h-1/2`}
      >
        {items.map((itm, index) => (
